...
export function ReviewSection({
  ...
  return (
    <div className="pt-12 border-t border-brand-cream-300 space-y-8">
      {/* Top Header & Metrics */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 pb-8 border-b border-brand-cream-200">
        {/* Left: Rating Summary */}
        <div className="space-y-3">
          <h3 className="font-serif font-extrabold text-2xl sm:text-3xl text-brand-forest-950">
            Customer Reviews & Ratings
          </h3>
          <div className="flex items-center gap-4">
            <div className="text-4xl font-extrabold text-brand-forest-950 font-serif">
              {avgRating}
            </div>
            <div>
        ...
        <div className="w-full lg:max-w-xs space-y-1.5 bg-brand-cream-50 p-4 rounded-2xl border border-brand-cream-300">
          {[5, 4, 3, 2, 1].map((stars) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-2 text-xs py-0.5 rounded-lg px-1.5 transition-colors ${
                  isSelected
                    ? 'bg-brand-forest-100 font-bold text-brand-forest-900'
                    : 'hover:bg-brand-cream-200/60 text-brand-charcoal-700'
                }`}
              >
                ...
                <div className="flex-1 bg-brand-cream-300 rounded-full h-2 overflow-hidden">
                  <div
                    className="bg-brand-forest-800 h-full rounded-full transition-all duration-500"
                    style={{ width: `${percent}%` }}
                  />
                </div>

                <span className="w-10 text-right text-[11px] text-brand-charcoal-500">
                  {percent}%
                </span>
              </button>
            );
      ...
      <div className="space-y-4">
        {displayedReviews.length === 0 ? (
          <div className="py-12 text-center text-brand-charcoal-500 bg-brand-cream-50 rounded-3xl p-8 border border-brand-cream-300">
            {selectedStarFilter !== null
          ...
          <div className="divide-y divide-brand-cream-200">
            {displayedReviews.map((rev) => (
              <div key={rev.id} className="py-6 space-y-2">
                ...
                {rev.title && (
                  <h4 className="font-bold text-sm text-brand-charcoal-900 pt-1">
                    {rev.title}
                  </h4>
                )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-3xl p-6 sm:p-8 shadow-2xl z-10 animate-slide-up">
            ...
            <form onSubmit={handleSubmitReview} className="space-y-4 mt-5">
              ...
              <div className="pt-3 flex justify-end gap-3 border-t border-brand-cream-200">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
