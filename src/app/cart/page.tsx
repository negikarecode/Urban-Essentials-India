  ...
  if (items.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        <div className="w-20 h-20 rounded-full bg-brand-cream-200 flex items-center justify-center text-brand-charcoal-400 mx-auto mb-4">
          <ShoppingBag className="w-10 h-10" />
        </div>
        <h1 className="font-serif font-bold text-3xl text-brand-forest-950 mb-2">
          Your Shopping Cart is Empty
        </h1>
        <p className="text-sm text-brand-charcoal-600 max-w-md mx-auto mb-8">
          Looks like you haven&apos;t added any essentials yet. Explore our curated collections for school, college, and office.
        </p>
        <Link
          href="/products"
          className="inline-flex items-center gap-2 px-8 py-3.5 bg-brand-forest-800 hover:bg-brand-forest-900 text-white font-bold text-sm rounded-xl shadow-md transition-colors"
        >
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      <h1 className="font-serif font-extrabold text-3xl sm:text-4xl text-brand-forest-950 mb-8">
        Your Cart ({items.length} unique item{items.length !== 1 ? 's' : ''})
      </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {items.map((item) => (
              <div key={item.id} className="p-4 sm:p-6 flex flex-col sm:flex-row gap-4 sm:items-center">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-serif font-bold text-base text-brand-charcoal-900 hover:text-brand-forest-800">
          ...
          <div className="bg-brand-cream-100/70 p-5 rounded-3xl border border-brand-cream-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {COUPONS_DATA.map((c) => (
                <div
                  key={c.id}
                  className="p-3 bg-white rounded-xl border border-brand-cream-300 flex flex-col justify-between"
                >
                  <div>
        ...
        <div className="lg:col-span-4 p-6 bg-white rounded-3xl border border-brand-cream-300 shadow-xs space-y-6 sticky top-28">
            ...
            {appliedCoupon ? (
              ...
              <form onSubmit={handleApplyCoupon} className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 text-xs rounded-xl border border-brand-cream-400 focus:outline-none focus:ring-1 focus:ring-brand-forest-800 uppercase"
                />
                <button
                  type="submit"
          ...
          <Link
            href="/checkout"
            className="w-full py-4 px-6 bg-brand-forest-800 hover:bg-brand-forest-900 text-white text-sm font-bold rounded-2xl shadow-lg flex items-center justify-center gap-2 group transition-all"
          >
